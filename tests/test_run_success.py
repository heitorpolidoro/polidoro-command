...
def test_run_success_with_arguments(parser, command_with_arguments, capsys):
    assert_call(parser, "", "usage: testCommand [-h] {command_test}\n", capsys)
    assert_call(parser, "command_test PO",
                "'command called with PO, default_ko, (), {}'\n",
                capsys, expected_exception=None)
    assert_call(parser, "command_test PO ARG1 ARG2 --ko=KO --kw1=KW2",
                '"command called with PO, KO, (\'ARG1\', \'ARG2\'), {\'kw1\': \'KW2\'}"\n',
                capsys, expected_exception=None)
